...
const handleClerkWebhook = httpAction(async (ctx, request) => {
  ...
  let event: Record<string, any>; // Using Record<string, any> for flexibility
  ...
  if (eventType === "user.created" || eventType === "user.updated") {
    ...
    if (!primaryEmail) {
      ...
      // Decide how to handle this - maybe skip or log an error
    ...
    try {
      ...
      // Return an internal server error response if the mutation fails
    ...
    return new Response("Unsupported event type", { status: 200 }); // Still OK, just ignored
  }
});
