...
- [x] Implement anonymous session store in `src/lib/utils/tracking.ts`:
  ...
  - [ ] Add functions to check if session is anonymous vs. claimed
...
- [ ] Implement "Claim Session" button/flow:
  ...
  - [ ] Hide claim button when already authenticated
  - [ ] Show regular profile menu when authenticated
- [ ] Create session migration utilities:
  - [ ] Functions to associate anonymous data with authenticated user
  - [ ] Conflict resolution strategy for merging data
  - [ ] Rollback capability if claiming fails
- [ ] Wrap the SvelteKit root layout (`src/routes/+layout.svelte`) with the Clerk provider (`<ClerkProvider>`). You might need to adjust your root layout to handle potential SSR limitations if any arise, but Clerk's SDK is generally client-side friendly.
- [ ] Implement basic sign-in and sign-up routes/components using Clerk's managed components (e.g., `<SignIn>`, `<SignUp>`) or build custom flows using Clerk hooks. Place these within your SvelteKit routing structure (e.g., `src/routes/sign-in/+page.svelte`).
